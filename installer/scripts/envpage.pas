{ ===========================================================================
  envpage.pas — custom wizard pages collecting the 12 environment settings.
  ...
  HeadlessCheck, WarmCheck, Pf2eCheck) are declared in the main script's
  global var block.
  =========================================================================== }
...
begin
  ...
  Lbl.Top := Y;
  Lbl.AutoSize := True;
  Lbl.Caption := 'Game system — pre-warm its monster compendium at startup:';

  Y := Y + ScaleY(20);
  Pf2eCheck := TNewCheckBox.Create(BehaviorPage);
  Pf2eCheck.Parent := BehaviorPage.Surface;
  Pf2eCheck.Top := Y;
  Pf2eCheck.Width := BehaviorPage.SurfaceWidth;
  Pf2eCheck.Caption := 'Pathfinder 2e (Monster Core bestiary)';
  Pf2eCheck.Checked := True;
...
var
  ...
  Path, ForgeProfile, Packs: String;
begin
  ForgeProfile := Trim(ForgePage.Values[0]);
  if ForgeProfile = '' then
    ForgeProfile := ExpandConstant('{app}\.puppeteer-profile');

  { WARM_PHASE2_PACKS is derived from the game-system checkboxes, not typed. }
  Packs := '';
  if Pf2eCheck.Checked then Packs := 'pf2e.pathfinder-monster-core';
  ...
  Lines[9]  := 'WARM_PHASE2_PACKS=' + Packs;
  Lines[10] := 'ALLOW_EVAL=' + BoolStr(SecurityPage.Values[0]);
  Lines[11] := 'FORGE_MODE=' + BoolStr(SecurityPage.Values[1]);
  Lines[12] := 'FORGE_PROFILE_DIR=' + ForgeProfile;
  Lines[13] := 'FORGE_MANUAL_LOGIN_TIMEOUT_MS=' + Trim(ForgePage.Values[1]);
  Lines[14] := 'PUPPETEER_EXECUTABLE_PATH=' + ExpandConstant('{app}\chromium\chrome.exe');
  Lines[15] := 'PUPPETEER_CACHE_DIR=' + ExpandConstant('{app}\chromium');
