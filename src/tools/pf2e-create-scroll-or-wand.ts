...
import {
  pf2eCreateScrollOrWandBody,
  type Pf2eCreateScrollOrWandResult,
} from '../evaluators/pf2e-create-scroll-or-wand.js';
import { jsonText, type ToolDefinition } from './types.js';

const Pf2eCreateScrollOrWandInput = z
  .object({
    actorId: z
      .string()
      .min(1)
...
export const pf2eCreateScrollOrWandTool: ToolDefinition<typeof Pf2eCreateScrollOrWandInput> = {
  ...
  inputSchema: Pf2eCreateScrollOrWandInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(
      pf2eCreateScrollOrWandBody,
      args,
    )) as Pf2eCreateScrollOrWandResult;
    if (!result.ok) {
