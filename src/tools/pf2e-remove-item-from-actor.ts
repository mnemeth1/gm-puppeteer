...
import {
  pf2eRemoveItemFromActorBody,
  type Pf2eRemoveItemFromActorResult,
} from '../evaluators/pf2e-remove-item-from-actor.js';
 ...
 */
const Pf2eRemoveItemFromActorInput = z
  .object({
    actorId: z
      .string()
      .min(1)
...
export const pf2eRemoveItemFromActorTool: ToolDefinition<typeof Pf2eRemoveItemFromActorInput> = {
  ...
  inputSchema: Pf2eRemoveItemFromActorInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(
      pf2eRemoveItemFromActorBody,
      args,
    )) as Pf2eRemoveItemFromActorResult;
    if (!result.ok) {
