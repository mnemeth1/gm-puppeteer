...
import {
  pf2eGetAvailableConditionsBody,
  type Pf2eGetAvailableConditionsResult,
} from '../evaluators/pf2e-get-available-conditions.js';
import { jsonText, type ToolDefinition } from './types.js';

const Pf2eGetAvailableConditionsInput = z.object({}).strict();

export const pf2eGetAvailableConditionsTool: ToolDefinition<
  typeof Pf2eGetAvailableConditionsInput
> = {
  ...
  inputSchema: Pf2eGetAvailableConditionsInput,
  async handler(_input, ctx) {
    const { page } = await ctx.browser.ensureStarted();
    const result = (await page.evaluate(
      pf2eGetAvailableConditionsBody,
    )) as Pf2eGetAvailableConditionsResult;
    if (!result.ok) {
