...
import {
  pf2eGetCreatureDetailsBody,
  SUPPORTED_CREATURE_TYPES,
  type Pf2eGetCreatureDetailsResult,
} from '../evaluators/pf2e-get-creature-details.js';
import { jsonText, type ToolDefinition } from './types.js';

const KNOWN_TYPES = new Set<string>(SUPPORTED_CREATURE_TYPES);

const Pf2eGetCreatureDetailsInput = z
  .object({
    uuid: z
      .string()
      .min(1)
      .describe(
        'Full Foundry Actor UUID. Two shapes are accepted uniformly: ' +
          ...
          '(the value returned by pf2e_search_compendium when searching actor packs). Resolves via ' +
          ...
          'loot / vehicle (use pf2e_get_actor_state for PCs).',
      ),
    descriptionFormat: z
      .enum(['html', 'text', 'both'])
      .optional()
...
export const pf2eGetCreatureDetailsTool: ToolDefinition<typeof Pf2eGetCreatureDetailsInput> = {
  name: 'pf2e_get_creature_details',
  description:
    ...
    "actors (type 'character') — use pf2e_get_actor_state for those, which surfaces runtime state " +
    ...
    'of Nethys at https://2e.aonprd.com/ via web-fetch. Companion to pf2e_search_compendium for ' +
  ...
  inputSchema: Pf2eGetCreatureDetailsInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(
      pf2eGetCreatureDetailsBody,
      args,
    )) as Pf2eGetCreatureDetailsResult;
    if (!result.ok) {
    ...
    if (!KNOWN_TYPES.has(result.type)) {
      ctx.log.warn(
        ...
        'pf2e_get_creature_details: actor type fell outside supported set — projection may be incomplete',
      );
    }
