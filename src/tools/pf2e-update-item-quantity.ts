...
import {
  pf2eUpdateItemQuantityBody,
  type Pf2eUpdateItemQuantityResult,
} from '../evaluators/pf2e-update-item-quantity.js';
 ...
 */
const Pf2eUpdateItemQuantityInput = z
  .object({
    actorId: z
      .string()
      .min(1)
...
export const pf2eUpdateItemQuantityTool: ToolDefinition<typeof Pf2eUpdateItemQuantityInput> = {
  ...
  description:
    ...
    'dedicated tools.',
  inputSchema: Pf2eUpdateItemQuantityInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(
      pf2eUpdateItemQuantityBody,
      args,
    )) as Pf2eUpdateItemQuantityResult;
    if (!result.ok) {
