...
import {
  setConditionValueBody,
  type SetConditionValueResult,
} from '../evaluators/set-condition-value.js';
 ...
 */
const SetConditionValueInput = z
  .object({
    actorId: z
      .string()
      .min(1)
...
export const setConditionValueTool: ToolDefinition<typeof SetConditionValueInput> = {
  ...
  inputSchema: SetConditionValueInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(setConditionValueBody, args)) as SetConditionValueResult;
    if (!result.ok) {
