...
import {
  ...
  type GetItemDetailsResult,
} from '../evaluators/get-item-details.js';
import { jsonText, type ToolDefinition } from './types.js';

const KNOWN_PROJECTIONS = new Set<string>(PROJECTED_ITEM_TYPES);

const GetItemDetailsInput = z
  .object({
    uuid: z
      .string()
      .min(1)
...
export const getItemDetailsTool: ToolDefinition<typeof GetItemDetailsInput> = {
  ...
  inputSchema: GetItemDetailsInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(getItemDetailsBody, args)) as GetItemDetailsResult;
    if (!result.ok) {
