...
import {
  pf2eUpdateItemUsesBody,
  type Pf2eUpdateItemUsesResult,
} from '../evaluators/pf2e-update-item-uses.js';
import { jsonText, type ToolDefinition } from './types.js';

/**
 * Schema for `pf2e_update_item_uses`. Single-shape input: actorId, itemId,
 * value (non-negative integer). Companion to `pf2e_use_item` (decrement
 * with side effects) and `pf2e_update_item_quantity` (sets quantity, not
 * uses).
 *
 * Boundary validation:
 ...
 *     deliberate divergence from pf2e_update_item_quantity, which rejects
 *     0 with a pointer to pf2e_remove_item_from_actor.
 ...
 */
const Pf2eUpdateItemUsesInput = z
  .object({
    actorId: z
      .string()
      .min(1)
    ...
    value: z
      ...
      .describe(
        'Absolute charge count to set in `system.uses.value`. Must be a non-negative integer. ' +
          ...
          'pf2e_remove_item_from_actor if you also want to delete the item). No upper bound is ' +
          'enforced — Foundry accepts values above `uses.max` (response carries `usesMax` so ' +
          'callers can detect over-set). Use pf2e_get_item_details to read the current `uses.value` ' +
          'and `uses.max` first.',
      ),
  })
  .strict();

export const pf2eUpdateItemUsesTool: ToolDefinition<typeof Pf2eUpdateItemUsesInput> = {
  name: 'pf2e_update_item_uses',
  description:
    'Set the absolute `system.uses.value` of a charges-tracking physical item on a world actor. ' +
    'Sibling to pf2e_use_item (which decrements with side effects: chat card, autoDestroy) and to ' +
    'pf2e_update_item_quantity (which sets `system.quantity`). Useful for refreshing daily wand ' +
    'charges, hand-tuning probe state, or reverting an accidental pf2e_use_item call. ' +
    ...
    'use pf2e_get_item_details to inspect the tracker first. ' +
    'Important nuance: for `consumable` items with `uses.max === 1` (potions, single-shot ' +
    'scrolls), the live counter that pf2e_use_item decrements is `system.quantity`, NOT ' +
    '`system.uses.value`. Setting `uses.value` on those items is technically valid but rarely ' +
    'the right verb — reach for pf2e_update_item_quantity instead. The cleanest signal that ' +
    ...
    '(that pipeline only fires inside ConsumablePF2e.consume()). Use pf2e_remove_item_from_actor to ' +
    'delete. ' +
  ...
  inputSchema: Pf2eUpdateItemUsesInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(pf2eUpdateItemUsesBody, args)) as Pf2eUpdateItemUsesResult;
    if (!result.ok) {
