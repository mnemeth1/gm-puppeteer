...
import {
  pf2eGetActorStateBody,
  type Pf2eGetActorStateResult,
} from '../evaluators/pf2e-get-actor-state.js';
 ...
 * trivially satisfied.
 */
const Pf2eGetActorStateInput = z
  .object({
    actorId: z
      .string()
      .min(1)
...
export const pf2eGetActorStateTool: ToolDefinition<typeof Pf2eGetActorStateInput> = {
  ...
  inputSchema: Pf2eGetActorStateInput,
  async handler(input, ctx) {
    ...
    const result = (await page.evaluate(pf2eGetActorStateBody, args)) as Pf2eGetActorStateResult;
    if (!result.ok) {
