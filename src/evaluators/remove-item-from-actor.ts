 ...
 */
export interface RemoveItemFromActorInput {
  actorId: string;
  itemId: string;
...
export type RemoveItemFromActorOk =
  | {
      ok: true;
...
export interface RemoveItemFromActorErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export async function removeItemFromActorBody(
  input: RemoveItemFromActorInput,
): Promise<RemoveItemFromActorResult> {
