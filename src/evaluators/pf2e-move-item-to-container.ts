 ...
 */
export interface Pf2eMoveItemToContainerInput {
  actorId: string;
  itemId: string;
...
export interface Pf2eMoveItemToContainerMovedItem {
  id: string;
  name: string;
  type: string;
  quantity: number;
...
export interface Pf2eMoveItemToContainerMergedInto {
  id: string;
  name: string;
  type: string;
  qtyBefore: number;
...
export type Pf2eMoveItemToContainerOk =
  | {
      ok: true;
      actor: { id: string; name: string };
      operation: 'moved';
      item: Pf2eMoveItemToContainerMovedItem;
    }
  | {
      ok: true;
      actor: { id: string; name: string };
      operation: 'merged';
      mergedInto: Pf2eMoveItemToContainerMergedInto;
    };

export interface Pf2eMoveItemToContainerErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eMoveItemToContainerResult = Pf2eMoveItemToContainerOk | Pf2eMoveItemToContainerErr;
...
export async function pf2eMoveItemToContainerBody(
  input: Pf2eMoveItemToContainerInput,
): Promise<Pf2eMoveItemToContainerResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eMoveItemToContainerErr => ({
    ok: false,
