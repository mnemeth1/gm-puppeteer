 ...
 */
export interface Pf2eRemoveConditionInput {
  actorId: string;
...
export interface Pf2eRemoveConditionCascadeEntry {
  id: string;
  slug: string;
  name: string;
  grantedBy: string;
...
export interface Pf2eRemoveConditionRemoved {
  ok: true;
  ...
  cascadeDeleted?: Pf2eRemoveConditionCascadeEntry[];
}

export interface Pf2eRemoveConditionDecremented {
  ok: true;
  actor: { id: string; name: string };
...
export interface Pf2eRemoveConditionNoop {
  ok: true;
  actor: { id: string; name: string };
  operation: 'noop';
  slug: string;
...
export type Pf2eRemoveConditionOk =
  | Pf2eRemoveConditionRemoved
  | Pf2eRemoveConditionDecremented
  | Pf2eRemoveConditionNoop;

export interface Pf2eRemoveConditionErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eRemoveConditionResult = Pf2eRemoveConditionOk | Pf2eRemoveConditionErr;
...
export async function pf2eRemoveConditionBody(
  input: Pf2eRemoveConditionInput,
): Promise<Pf2eRemoveConditionResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eRemoveConditionErr => ({
    ok: false,
  ...
  const cascadeDeleted: Pf2eRemoveConditionCascadeEntry[] = [];
  {
  ...
  // -- Build response.
  if (willRemove) {
    const result: Pf2eRemoveConditionRemoved = {
      ok: true,
      actor: { id: actor.id ?? input.actorId, name: actor.name ?? '' },
      operation: 'removed',
      condition: {
