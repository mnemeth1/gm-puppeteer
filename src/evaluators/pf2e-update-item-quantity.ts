 ...
 */
export interface Pf2eUpdateItemQuantityInput {
  actorId: string;
  itemId: string;
  quantity: number;
}

export interface Pf2eUpdateItemQuantityItem {
  id: string;
  name: string;
  type: string;
  qtyBefore: number;
...
export interface Pf2eUpdateItemQuantityOk {
  ok: true;
  actor: { id: string; name: string };
  operation: 'updated';
  item: Pf2eUpdateItemQuantityItem;
}

export interface Pf2eUpdateItemQuantityErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eUpdateItemQuantityResult = Pf2eUpdateItemQuantityOk | Pf2eUpdateItemQuantityErr;
...
export async function pf2eUpdateItemQuantityBody(
  input: Pf2eUpdateItemQuantityInput,
): Promise<Pf2eUpdateItemQuantityResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eUpdateItemQuantityErr => ({
    ok: false,
