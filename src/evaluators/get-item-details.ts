...
export interface GetItemDetailsOk {
  ok: true;
  uuid: string;
  id: string;
  name: string;
...
export interface GetItemDetailsErr {
  ok: false;
  error: {
...
export type GetItemDetailsResult = GetItemDetailsOk | GetItemDetailsErr;
...
export async function getItemDetailsBody(
  input: GetItemDetailsInput,
): Promise<GetItemDetailsResult> {
