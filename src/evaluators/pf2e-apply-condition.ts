 ...
 */
export interface Pf2eApplyConditionInput {
  actorId: string;
  slug: string;
  value: number | null;
}

export interface Pf2eApplyConditionCascadeEntry {
  id: string;
  slug: string;
  name: string;
  grantedBy: string;
...
export interface Pf2eApplyConditionApplied {
  ok: true;
  ...
  cascadeGranted?: Pf2eApplyConditionCascadeEntry[];
}

export interface Pf2eApplyConditionNoop {
  ok: true;
  actor: { id: string; name: string };
  operation: 'noop';
  condition: {
...
export type Pf2eApplyConditionOk = Pf2eApplyConditionApplied | Pf2eApplyConditionNoop;

export interface Pf2eApplyConditionErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eApplyConditionResult = Pf2eApplyConditionOk | Pf2eApplyConditionErr;
...
export async function pf2eApplyConditionBody(
  input: Pf2eApplyConditionInput,
): Promise<Pf2eApplyConditionResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eApplyConditionErr => ({
    ok: false,
  ...
  function buildAppliedResponse(args: {
    ...
    valueApplied: number | null;
    clamped: boolean;
  }): Pf2eApplyConditionApplied {
    const conditions = args.actor.itemTypes?.condition ?? [];
    const cascadeGranted: Pf2eApplyConditionCascadeEntry[] = [];
    if (args.created.id) {
    ...
    const result: Pf2eApplyConditionApplied = {
      ok: true,
