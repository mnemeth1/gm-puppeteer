 ...
 */
export interface Pf2eRollCheckInput {
  actorId: string;
...
export interface Pf2eRollCheckOk {
  ok: true;
...
export interface Pf2eRollCheckErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eRollCheckResult = Pf2eRollCheckOk | Pf2eRollCheckErr;

export async function pf2eRollCheckBody(input: Pf2eRollCheckInput): Promise<Pf2eRollCheckResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eRollCheckErr => ({
    ok: false,
