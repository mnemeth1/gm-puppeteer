 ...
 */
export interface Pf2eUseItemInput {
  actorId: string;
  itemId: string;
}

export interface Pf2eUseItemResultItem {
  id: string;
  name: string;
  type: string;
...
export interface Pf2eUseItemOk {
  ok: true;
...
export type Pf2eUseItemResult = Pf2eUseItemOk | Pf2eUseItemErr;
...
export async function pf2eUseItemBody(input: Pf2eUseItemInput): Promise<Pf2eUseItemResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eUseItemErr => ({
    ok: false,
  ...
  if (targetType === 'consumable') {
    ...
    const item: Pf2eUseItemResultItem = {
      id: targetIdBefore,
      name: targetName,
      type: targetType,
      subtype,
  ...
  const eqItem: Pf2eUseItemResultItem = {
    id: eqTargetId,
    name: targetName,
    type: targetType,
    subtype,
