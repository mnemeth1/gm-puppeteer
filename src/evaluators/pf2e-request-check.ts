 ...
 */
export interface Pf2eRequestCheckInput {
  actorId: string;
...
export interface Pf2eRequestCheckOk {
  ok: true;
...
export interface Pf2eRequestCheckErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eRequestCheckResult = Pf2eRequestCheckOk | Pf2eRequestCheckErr;

export async function pf2eRequestCheckBody(
  input: Pf2eRequestCheckInput,
): Promise<Pf2eRequestCheckResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eRequestCheckErr => ({
    ok: false,
