 ...
 */
export interface SetConditionValueInput {
  actorId: string;
  slug: string;
  value: number;
}

export interface SetConditionValueCascadeEntry {
  id: string;
  slug: string;
  name: string;
  grantedBy: string;
...
export interface SetConditionValueApplied {
  ok: true;
  ...
  cascadeGranted?: SetConditionValueCascadeEntry[];
}

export interface SetConditionValueNoop {
  ok: true;
  actor: { id: string; name: string };
  operation: 'noop';
  condition: {
...
export type SetConditionValueOk = SetConditionValueApplied | SetConditionValueNoop;

export interface SetConditionValueErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export async function setConditionValueBody(
  input: SetConditionValueInput,
): Promise<SetConditionValueResult> {
  ...
  const cascadeGranted: SetConditionValueCascadeEntry[] = [];
  ...
  const response: SetConditionValueApplied = {
    ok: true,
    actor: { id: actor.id ?? input.actorId, name: actor.name ?? '' },
    operation: 'applied',
    condition: {
