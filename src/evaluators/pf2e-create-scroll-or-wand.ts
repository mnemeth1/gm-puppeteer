 ...
 */
export interface Pf2eCreateScrollOrWandInput {
  actorId: string;
  spellUuid: string;
  kind: 'scroll' | 'wand';
  rank: number;
...
export interface Pf2eCreateScrollOrWandCascadeEntry {
  id: string;
  uuid: string;
  name: string;
  type: string;
...
export interface Pf2eCreateScrollOrWandItem {
  id: string;
  uuid: string;
  name: string;
  type: 'consumable';
...
export interface Pf2eCreateScrollOrWandOk {
  ok: true;
  actor: { id: string; name: string };
  item: Pf2eCreateScrollOrWandItem;
  cascadeGranted?: Pf2eCreateScrollOrWandCascadeEntry[];
  warnings?: string[];
}

export interface Pf2eCreateScrollOrWandErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eCreateScrollOrWandResult = Pf2eCreateScrollOrWandOk | Pf2eCreateScrollOrWandErr;

declare function fromUuid(uuid: string): Promise<unknown>;

export async function pf2eCreateScrollOrWandBody(
  input: Pf2eCreateScrollOrWandInput,
): Promise<Pf2eCreateScrollOrWandResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eCreateScrollOrWandErr => ({
    ok: false,
  ...
  const cascadeGranted: Pf2eCreateScrollOrWandCascadeEntry[] = [];
  ...
  const result: Pf2eCreateScrollOrWandOk = {
    ok: true,
