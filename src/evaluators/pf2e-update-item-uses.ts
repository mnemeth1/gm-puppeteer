 ...
 */
export interface Pf2eUpdateItemUsesInput {
  actorId: string;
  itemId: string;
  value: number;
}

export interface Pf2eUpdateItemUsesItem {
  id: string;
  name: string;
  type: string;
  usesBefore: number;
...
export interface Pf2eUpdateItemUsesOk {
  ok: true;
  actor: { id: string; name: string };
  operation: 'updated';
  item: Pf2eUpdateItemUsesItem;
}

export interface Pf2eUpdateItemUsesErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eUpdateItemUsesResult = Pf2eUpdateItemUsesOk | Pf2eUpdateItemUsesErr;
...
export async function pf2eUpdateItemUsesBody(
  input: Pf2eUpdateItemUsesInput,
): Promise<Pf2eUpdateItemUsesResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eUpdateItemUsesErr => ({
    ok: false,
