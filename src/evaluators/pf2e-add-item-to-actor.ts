 ...
 */
export interface Pf2eAddItemToActorInput {
  actorId: string;
  sourceUuid: string;
  quantity: number;
...
export interface Pf2eAddItemToActorCreatedItem {
  id: string;
  uuid: string;
  name: string;
  type: string;
...
export interface Pf2eAddItemToActorCascadeEntry {
  id: string;
  uuid: string;
  name: string;
  type: string;
...
export interface Pf2eAddItemToActorMergedInto {
  id: string;
  uuid: string;
  name: string;
...
export interface Pf2eAddItemToActorOk {
  ok: true;
  actor: { id: string; name: string };
  operation: 'created' | 'merged';
  item?: Pf2eAddItemToActorCreatedItem;
  cascadeGranted?: Pf2eAddItemToActorCascadeEntry[];
  mergedInto?: Pf2eAddItemToActorMergedInto;
  warnings?: string[];
}

export interface Pf2eAddItemToActorErr {
  ok: false;
  error: {
    code: 'INVALID_INPUT';
    message: string;
...
export type Pf2eAddItemToActorResult = Pf2eAddItemToActorOk | Pf2eAddItemToActorErr;
...
export async function pf2eAddItemToActorBody(
  input: Pf2eAddItemToActorInput,
): Promise<Pf2eAddItemToActorResult> {
  ...
  const fail = (message: string, details: Record<string, unknown>): Pf2eAddItemToActorErr => ({
    ok: false,
  ...
  if (candidateLookup.match) {
    ...
    const matchContainer =
      typeof matchContainerRaw === 'string' && matchContainerRaw.length > 0
        ? matchContainerRaw
        : null;
    const result: Pf2eAddItemToActorOk = {
      ok: true,
  ...
  const cascadeGranted: Pf2eAddItemToActorCascadeEntry[] = [];
