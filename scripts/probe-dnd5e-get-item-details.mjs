...
try {
  ...
  check(
    'P1 CONFIG.Item.dataModels covers the 13 expected types',
  ...
  check(
    'P1 weapon price is {value, denomination, valueInGP}',
    ['denomination', 'value', 'valueInGP'].every((k) => shapes.weapon.priceKeys.includes(k)),
    shapes.weapon.priceKeys,
  );
  check(
    'P1 weapon uses carries spent/max/recovery/value',
  ...
  check(
    'P1 physical/non-physical split: weapon has quantity, spell does not',
    shapes.weapon.hasQuantity && !shapes.spell.hasQuantity,
  );
  check('P1 spell has no system.type field', !shapes.spell.hasTypeField);
  check(
    'P1 class system.advancement is a Map-like AdvancementCollection',
  ...
  check(
    'S1 weapon ok + weapon block + physical block',
    s1.ok &&
      !!s1.weapon &&
      !!s1.physical &&
      s1.type === 'weapon' &&
      Array.isArray(s1.weapon.damage.parts) &&
      s1.weapon.damage.parts.length > 0 &&
      s1.physical.priceFormatted.length > 0,
    s1.ok ? { name: s1.name, weapon: s1.weapon, physical: s1.physical, uses: s1.uses } : s1,
  );
  check(
    'S1 weapon damage.base.types resolved from Set',
  ...
  check(
    'S2 equipment ok + armor projection + physical block',
    s2.ok &&
      !!s2.equipment &&
      s2.equipment.armor &&
      typeof s2.equipment.armor.value === 'number' &&
      !!s2.physical &&
      s2.rarity === 'legendary',
  ...
  check(
    'S3 consumable ok + consumable block + physical block',
  ...
  check(
    'S3b spell scroll → consumableType "scroll"',
  ...
  check(
    'S4 tool ok + tool block (ability=int) + physical block',
  ...
  check(
    'S5 loot ok + loot block + physical block (equipped false)',
  ...
  check(
    'S6 container ok + capacity/currency + physical block + no uses',
    s6.ok &&
      !!s6.container &&
      !!s6.container.capacity &&
      !!s6.container.currency &&
      !!s6.physical &&
      s6.uses === undefined,
  ...
  check(
    'S7 spell ok + spell block (level 4, school trs) + NO physical block',
    s7.ok &&
      !!s7.spell &&
      s7.spell.level === 4 &&
      s7.spell.school === 'trs' &&
      !!s7.spell.components &&
      s7.physical === undefined,
  ...
  check(
    'S8 feat ok + feat block (featType, advancement summary)',
    s8.ok &&
      !!s8.feat &&
      typeof s8.feat.featType === 'string' &&
      Array.isArray(s8.feat.advancement),
  ...
  check(
    'S9 background ok + startingEquipment array + advancement',
    s9.ok &&
      !!s9.background &&
      Array.isArray(s9.background.startingEquipment) &&
  ...
  check(
    'S10 class ok + hitDice + spellcasting + non-empty advancement summary',
    s10.ok &&
      !!s10.class &&
      s10.class.hitDice === 'd6' &&
      !!s10.class.spellcasting &&
      Array.isArray(s10.class.advancement) &&
      s10.class.advancement.length > 0,
  ...
  check(
    'S11 subclass ok + classIdentifier "sorcerer"',
  ...
  check(
    'S12 race ok + creatureType + senses (darkvision 60)',
    s12.ok &&
      !!s12.race &&
      s12.race.creatureType?.value === 'humanoid' &&
  ...
  if (facilityUuid) {
    ...
    check(
      'S13 facility ok + facility block projected',
  ...
  check(
    'S14b Actor UUID → WRONG_DOCUMENT_TYPE',
    !s14b.ok && s14b.error.code === 'WRONG_DOCUMENT_TYPE',
    s14b,
  );
  ...
  check(
    'S15 descriptionFormat html → description only',
    s15h.ok && s15h.description !== undefined && s15h.descriptionText === undefined,
  );
  check(
    'S15 descriptionFormat text → descriptionText only',
  ...
  check(
    'S16 includeRawSystem → rawSystem object present',
    s16.ok && s16.rawSystem !== undefined && typeof s16.rawSystem === 'object',
  );
  const s16b = await run(UUID.spell);
  check(
    'S16b rawSystem omitted by default for a projected type',
