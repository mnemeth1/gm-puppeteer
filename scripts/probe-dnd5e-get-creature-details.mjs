...
try {
  ...
  const shapes = await page.evaluate(async (uuids) => {
    ...
    return {
      ...
      weapon: {
        name: wpn?.name,
        activitiesIsMapLike: typeof acts?.getByType === 'function' && typeof acts?.size === 'number',
        activitiesObjKeysEmpty: Object.keys(acts || {}).length === 0,
        labelsPopulated:
          Array.isArray(wpn?.labels?.attacks) && wpn.labels.attacks.length > 0,
      },
      vehicle: {
        type: veh.type,
  ...
  check('P1 actor data models include npc + vehicle',
  ...
  check('P1 vehicle hp carries dt (damage threshold)',
    shapes.vehicle.hpKeys.includes('dt'), shapes.vehicle.hpKeys);
  ...
  check('S1 NPC ok + npc block populated',
    s1.ok && !!s1.npc && typeof s1.npc.ac.value === 'number' && s1.npc.hp.max > 0 &&
      s1.npc.abilities.str.score > 0 && Array.isArray(s1.npc.saves) && s1.npc.saves.length === 6,
    s1.ok ? { name: s1.name, cr: s1.cr, ac: s1.npc.ac, hp: s1.npc.hp } : s1);
  check('S1 NPC attacks projected from item labels',
    s1.ok && s1.npc.attacks.length > 0 && typeof s1.npc.attacks[0].attackBonus === 'string',
    s1.ok ? s1.npc.attacks : null);
  check('S1 NPC features projected (Nimble Escape)',
    s1.ok && s1.npc.features.some((f) => /nimble/i.test(f.name)),
    s1.ok ? s1.npc.features.map((f) => f.name) : null);
  check('S1 NPC creatureType + size populated',
  ...
  check('S2 2024-ruleset NPC parses identically',
  ...
  check('S3 caster spellcasting populated',
    s3.ok && s3.npc?.spellcasting && s3.npc.spellcasting.ability === 'int' &&
      s3.npc.spellcasting.saveDc > 0 && s3.npc.spellcasting.slots.length > 0 &&
  ...
  check('S4 damage interactions resolve Sets to arrays',
    s4.ok && s4.npc.damageInteractions.immunities.includes('poison') &&
  ...
  check('S5 vehicle ok + vehicle block populated',
    s5.ok && !!s5.vehicle && s5.vehicle.vehicleType.length > 0 && s5.vehicle.hp.max > 0 &&
      typeof s5.vehicle.hp.damageThreshold === 'number' && !!s5.vehicle.capacity,
  ...
  check('S6b Item UUID → WRONG_DOCUMENT_TYPE',
    !s6b.ok && s6b.error.code === 'WRONG_DOCUMENT_TYPE', s6b);
  if (pcUuid) {
    const s6c = await run(pcUuid);
    check('S6c PC actor → ACTOR_TYPE_UNSUPPORTED (points at dnd5e_get_actor_state)',
      !s6c.ok && s6c.error.code === 'ACTOR_TYPE_UNSUPPORTED' &&
  ...
  check('S7 descriptionFormat html → description only',
    s7h.ok && s7h.description !== undefined && s7h.descriptionText === undefined);
  check('S7 descriptionFormat text → descriptionText only',
  ...
  check('S8 includeEffects → effects array present',
    s8.ok && Array.isArray(s8.effects));
  check('S8 includeRawSystem → rawSystem object present',
