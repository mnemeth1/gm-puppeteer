...
import {
  ...
  type Pf2eCalculateEncounterBudgetResult,
} from '../../src/tools/pf2e-calculate-encounter-budget.js';
...
async function run(input: {
  ...
  partySize: number;
  difficulty: string;
}): Promise<Pf2eCalculateEncounterBudgetResult> {
  const ctx = makeCtx();
  const blocks = await pf2eCalculateEncounterBudgetTool.handler(
    input as Parameters<typeof pf2eCalculateEncounterBudgetTool.handler>[0],
    ctx,
  );
  expect(blocks).toHaveLength(1);
  expect(blocks[0]?.type).toBe('text');
  return JSON.parse((blocks[0] as { text: string }).text) as Pf2eCalculateEncounterBudgetResult;
}
...
describe('pf2e_calculate_encounter_budget', () => {
  ...
  it('zod rejects out-of-range and bad-enum inputs', () => {
    const schema = pf2eCalculateEncounterBudgetTool.inputSchema;
  ...
  it('zod rejects unexpected input keys (.strict())', () => {
    const parsed = pf2eCalculateEncounterBudgetTool.inputSchema.safeParse({
      partyLevel: 5,
      partySize: 4,
      difficulty: 'moderate',
      extra: 'nope',
