...
import { searchCompendiumTool } from '../../src/tools/search-compendium.js';
import type { BrowserSession } from '../../src/browser/session.js';
import type { Logger } from '../../src/logging.js';
import type { SearchCompendiumResult } from '../../src/evaluators/search-compendium.js';
...
describe('pf2e_search_compendium', () => {
  it('forwards a name query with default limit, returns results as text content', async () => {
    ...
    const blocks = await searchCompendiumTool.handler({ query: 'goblin' }, ctx);
  ...
  it('forwards every filter and omits absent ones', async () => {
    ...
    await searchCompendiumTool.handler(
      {
        query: 'wolf',
  ...
  it('refuses an empty input without scanning packs', async () => {
    ...
    const blocks = await searchCompendiumTool.handler({}, ctx);
  ...
  it('rejects an empty query string', () => {
    const parsed = searchCompendiumTool.inputSchema.safeParse({ query: '' });
    expect(parsed.success).toBe(false);
  });

  it('rejects an unsupported document type', () => {
    const parsed = searchCompendiumTool.inputSchema.safeParse({
      query: 'x',
      type: 'NotARealType',
    });
    expect(parsed.success).toBe(false);
  ...
  it('rejects an unsupported actorType', () => {
    const parsed = searchCompendiumTool.inputSchema.safeParse({ actorType: 'monster' });
    expect(parsed.success).toBe(false);
  });
  ...
  it('rejects a limit above the cap', () => {
    const parsed = searchCompendiumTool.inputSchema.safeParse({ query: 'x', limit: 1000 });
    expect(parsed.success).toBe(false);
  });

  it('rejects an empty traits array', () => {
    const parsed = searchCompendiumTool.inputSchema.safeParse({ traits: [] });
    expect(parsed.success).toBe(false);
  });

  it('rejects descriptionMatch shorter than 2 chars', () => {
    const parsed = searchCompendiumTool.inputSchema.safeParse({ descriptionMatch: 'a' });
    expect(parsed.success).toBe(false);
  });

  it('rejects unknown fields (strict)', () => {
    const parsed = searchCompendiumTool.inputSchema.safeParse({ query: 'x', extraField: 1 });
    expect(parsed.success).toBe(false);
  });
});
