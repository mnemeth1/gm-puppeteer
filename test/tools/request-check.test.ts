...
import { requestCheckTool } from '../../src/tools/request-check.js';
...
describe('request_check', () => {
  it('returns the evaluator result as a JSON content block on success', async () => {
    ...
    const result = await requestCheckTool.handler(
  ...
  it('normalizes omitted optional fields when calling the evaluator', async () => {
    ...
    await requestCheckTool.handler({ actorId: 'pc-1', checkType: 'stealth' }, ctx);
  ...
  it('throws ToolError(INVALID_INPUT) when the evaluator rejects a non-PC actor', async () => {
    ...
    const err = await requestCheckTool
  ...
  it('rejects an unknown checkType via the input schema', () => {
    const parsed = requestCheckTool.inputSchema.safeParse({
      actorId: 'pc-1',
      checkType: 'vibes',
    });
